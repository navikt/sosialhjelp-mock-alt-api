package no.nav.sbl.sosialhjelp.mock.alt.otherEndpoints.feil

import no.nav.sbl.sosialhjelp.mock.alt.datastore.feil.FeilService
import no.nav.sbl.sosialhjelp.mock.alt.datastore.feil.Feilsituasjon
...
class FeilController(
    val feilService: FeilService,
) {
    @PostMapping("/feil")
    fun editFeil(
        @RequestBody feilsituasjoner: FeilsituasjonerFrontend,
    ) {
    ...
    ): ResponseEntity<FeilsituasjonerFrontend> = ResponseEntity.ok(FeilsituasjonerFrontend(ident, feilService.hentFeil(ident)))

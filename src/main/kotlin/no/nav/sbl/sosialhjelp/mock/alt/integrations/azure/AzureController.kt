package no.nav.sbl.sosialhjelp.mock.alt.integrations.azure
...
import no.nav.sbl.sosialhjelp.mock.alt.integrations.azure.model.AzureAdBruker
import no.nav.sbl.sosialhjelp.mock.alt.integrations.azure.model.AzureAdBrukere
import no.nav.sbl.sosialhjelp.mock.alt.integrations.azure.model.AzureAdGruppe
import no.nav.sbl.sosialhjelp.mock.alt.integrations.azure.model.AzureAdGrupper
...
import no.nav.sbl.sosialhjelp.mock.alt.utils.hentFnrFraToken
import no.nav.sbl.sosialhjelp.mock.alt.utils.hentFnrFraTokenNoDefault
...
class AzureController(
    ...
    fun getInnloggetAzureBrukersGrupper(
        @RequestHeader headers: HttpHeaders,
    ): ResponseEntity<AzureAdGrupper> {
    ...
    private fun hentGrupper(id: String): List<AzureAdGruppe> = rolleService.hentKonfigurasjon(id).map { toAzureRolle(it) }

    private fun toAzureRolle(rolle: AdminRolle): AzureAdGruppe =
        when (rolle) {
            AdminRolle.MODIA_VEILEDER ->
                AzureAdGruppe("0000-MOCK-sosialhjelp-modia-veileder", "Modia-Veiledere")
        }

    @GetMapping("/azuread/graph/groups/{gruppeId}/members")
    fun getAzureGruppeBrukere(
        ...
        return AzureAdBrukere(value = personaListe.map { AzureAdBruker(it) })
    }

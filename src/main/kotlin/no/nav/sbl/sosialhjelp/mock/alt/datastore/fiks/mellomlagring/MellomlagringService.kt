...
import no.nav.sbl.sosialhjelp.mock.alt.integrations.fiks.mellomlagring.MellomlagringDokumentInfo
import no.nav.sbl.sosialhjelp.mock.alt.integrations.fiks.mellomlagring.MellomlagringDto
...
class MellomlagringStorage {
    ...
    fun getAll(soknadId: String): MutableList<FileEntry>? = items[soknadId]

    fun find(
        soknadId: String,
        filId: String,
    ): FileEntry? = items[soknadId]?.find { it.filId == filId }

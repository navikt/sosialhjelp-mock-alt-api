package no.nav.sbl.sosialhjelp.mock.alt.datastore.norg

import no.nav.sbl.sosialhjelp.mock.alt.datastore.norg.model.NavEnhet
...
class NorgService {
    ...
    fun getNavenhet(enhetsnr: String): NavEnhet? =
        navEnheter[enhetsnr]
            ?: lagMockNavEnhet(enhetsnr, "Generert mockenhet $enhetsnr, Mock kommune")

    fun getNavEnhetForGt(geografiskTilknytning: String): NavEnhet =
        gtNavEnheter[geografiskTilknytning]
            ?: lagMockNavEnhet(
                geografiskTilknytning,
                "Generert mockenhet for gt $geografiskTilknytning, Mock kommune",
            )

    fun getAlleNavenheter(): Collection<NavEnhet> = navEnheter.values
}

...
class PdlAdresseSokService {
    ...
    init {
        ...
        hits?.let { hit -> nyAdresseMap.putAll(hit.map { it.vegadresse.adressenavn + it.vegadresse.husnummer + it.vegadresse.husbokstav to it }) }
    }

    fun getAdresse(sokestreng: String): PdlAdresseSokResponse {
        log.info("PDL adressesok, sokestreng: $sokestreng")
        val treff = nyAdresseMap.filter { (key, _) -> key.startsWith(sokestreng, ignoreCase = true) }.values

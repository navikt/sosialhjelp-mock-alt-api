...
class SkattbarInntekt(
    val oppgaveInntektsmottaker: MutableList<OppgaveInntektsmottaker> = mutableListOf()
) {
  data class Builder(var oppgaver: MutableList<OppgaveInntektsmottaker> = mutableListOf()) {
...
class OppgaveInntektsmottaker(
  ...
  data class Builder(
    ...
    fun standardOppgave(
      ...
      leggTilInntekt(Inntekt.Builder().type(Inntektstype.Loennsinntekt).beloep(belop).build())
...
class Inntekt(
    ...
    val aldersUfoereEtterlatteAvtalefestetOgKrigspensjon:
  ...
  fun type(): Inntektstype {
    return when {
      ...
      aldersUfoereEtterlatteAvtalefestetOgKrigspensjon != null ->
  ...
  data class Builder(
      ...
      var aldersUfoereEtterlatteAvtalefestetOgKrigspensjon:
          AldersUfoereEtterlatteAvtalefestetOgKrigspensjon? =
          null,
  ) {
    ...
    fun type(type: Inntektstype, subType: Inntektstype? = null) = apply {
      when (type) {
        Inntektstype.Loennsinntekt ->
            loennsinntekt = Loennsinntekt(Tilleggsinformasjon.Builder().ofType(subType).build())
        Inntektstype.YtelseFraOffentlige ->
            ytelseFraOffentlige =
                YtelseFraOffentlige(Tilleggsinformasjon.Builder().ofType(subType).build())
        Inntektstype.PensjonEllerTrygd ->
            pensjonEllerTrygd =
                PensjonEllerTrygd(Tilleggsinformasjon.Builder().ofType(subType).build())
...
class Loennsinntekt(val tilleggsinformasjon: Tilleggsinformasjon)

class YtelseFraOffentlige(val tilleggsinformasjon: Tilleggsinformasjon)

class PensjonEllerTrygd(val tilleggsinformasjon: Tilleggsinformasjon)

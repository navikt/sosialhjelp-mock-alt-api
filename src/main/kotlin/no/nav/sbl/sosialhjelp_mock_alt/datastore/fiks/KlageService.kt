...
class KlageService(private val soknadService: SoknadService) {
  ...
  fun leggTilKlage(fiksDigisosId: FiksDigisosId, klage: InputKlage) {
    val soknad = soknadService.hentSoknad(fiksDigisosId.value) ?: error("Ingen søknad for fiksDigisosId: ${fiksDigisosId}")
    val jsonDigisosSoker: JsonDigisosSoker = soknadService.hentDokument(soknad.fiksDigisosId, soknad.digisosSoker!!.metadata)?.let {
      objectMapper.readValue(it)
    } ?: error("Feeeekk")
    val vedtak = jsonDigisosSoker.hendelser.asSequence().filterIsInstance<JsonVedtakFattet>().filter { it.utfall == JsonVedtakFattet.Utfall.INNVILGET }.map { it.vedtaksfil.referanse }.filter {
      when (it) {
    ...
    val dokumentLagerRef = soknadService.leggInnIDokumentlager("klage", objectMapper.writeValueAsBytes(klage))
    val lagretKlage = Klage(fiksDigisosId.value, dokumentLagerRef, vedtakRef = vedtak, status = KlageStatus.UNDER_BEHANDLING, utfall = null)

...
class PdlService(
    ...
    fun getInnsynResponseFor(ident: String): PdlInnsynPersonResponse {
        ...
        var navnList: List<PdlPersonNavn> =
            listOf(PdlPersonNavn("Ukjent", "PDL", "Person"))
        if (personalia != null) {
            adressebeskyttelseList =
                listOf(Adressebeskyttelse(personalia.adressebeskyttelse))
            navnList =
                listOf(
                    PdlPersonNavn(
    ...
    fun getSoknadPersonResponseFor(ident: String): PdlSoknadPersonResponse {
        ...
        if (personalia != null) {
            ...
            if (personalia.sivilstand.equals("GIFT", true) ||
                ...
                sivilstand =
                    PdlSivilstand(
                        SivilstandType.valueOf(personalia.sivilstand),
                        personalia.ektefelleFnr
                    )
            }
    ...
    fun getSoknadBarnResponseFor(ident: String): PdlSoknadBarnResponse {
        ...
        return PdlSoknadBarnResponse(
            errors = null,
            data = PdlSoknadHentBarn(hentPerson = pdlBarn)
        )
    }
    ...
    private fun leggTilEktefelle(personalia: Personalia) {
        ...
        when (personalia.ektefelleType) {
            "EKTEFELLE_SAMME_BOSTED" ->
                ektefelleMap[fnr] =
                    ektefelleSammeBosted(personalia.ektefelleFodselsdato)

            "EKTEFELLE_ANNET_BOSTED" ->
                ektefelleMap[fnr] =
                    ektefelleAnnetBosted(personalia.ektefelleFodselsdato)

            "EKTEFELLE_MED_ADRESSEBESKYTTELSE" -> ektefelleMap[fnr] =
                ektefelleMedAdressebeskyttelse

            else -> ektefelleMap[fnr] =
                defaultEktefelle(personalia.ektefelleFodselsdato)
        }
    }
    ...
    fun getPersonalia(ident: String): Personalia {
        return personListe[ident]
            ?: throw MockAltException("Ident $ident not found!")
    }

    fun getBarn(ident: String): PdlSoknadBarn {
        return barnMap[ident]
            ?: throw MockAltException("Barn with ident $ident not found!")
    }
    ...
    private fun opprettBrukerMedAlt(
        ...
        ektefelleMap[brukerFnr] =
            ektefelleSammeBosted(standardBruker.ektefelleFodselsdato)
        ...
        eregService.putOrganisasjonNoekkelinfo(
            organisasjonsnummer,
            "Arbeidsgiveren AS"
        )
    ...
    private fun opprettNavKontaktsenterBruker(
        ...
        val trekk: Forskuddstrekk =
            Forskuddstrekk.Builder().beskrivelse("skattetrekk").beloep(3333)
                .build()
        val inntekt: Inntekt =
            Inntekt.Builder().type(Inntektstype.Loennsinntekt).beloep(18000)
                .build()
        ...
        val inntektoppgave =
            OppgaveInntektsmottaker.Builder()
                ...
                .kalendermaaned(
                    DateTimeFormatter.ofPattern("yyyy-MM").format(dato)
                )
                .leggTilForskuddstrekk(trekk)
                .leggTilInntekt(inntekt)
                .build()
        val skattbarInntekt =
            SkattbarInntekt.Builder().leggTilOppgave(inntektoppgave).build()

...
class FrontendLoggController {
  ...
  data class LogEntryRequest(val message: String)

  @PostMapping("/warn")
  fun logWarning(@RequestBody entry: LogEntryRequest) = log.warn(entry.message)

  @PostMapping("/info") fun logInfo(@RequestBody entry: LogEntryRequest) = log.info(entry.message)

  @PostMapping("/error")
  fun logError(@RequestBody entry: LogEntryRequest) = log.error(entry.message)
}

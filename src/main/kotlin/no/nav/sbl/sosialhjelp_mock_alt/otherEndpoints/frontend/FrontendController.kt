...
class FrontendController(
    ...
    fun frontendUpload(@RequestBody body: String): ResponseEntity<String> {
        ...
        personalia.skattetatenUtbetalinger.forEach {
            skattbarInntektBuilder.leggTilOppgave(
                FrontendSkattbarInntekt.oversettTilInntektsmottaker(
                    it
                )
            )
        }
        skatteetatenService.putSkattbarInntekt(
            personalia.fnr,
            skattbarInntektBuilder.build()
        )
        val bostotteDto = BostotteDto()
        personalia.bostotteSaker.forEach { bostotteDto.saker.add(it) }
        personalia.bostotteUtbetalinger.forEach {
            bostotteDto.utbetalinger.add(
                it
            )
        }
        ...
        rolleService.leggTilKonfigurasjon(
            personalia.fnr,
            personalia.administratorRoller
        )
    ...
    fun frontendDownload(@RequestParam ident: String): ResponseEntity<FrontendPersonalia> {
        ...
        val skattbarInntekt =
            skatteetatenService.getSkattbarInntekt(personalia.fnr)
        frontendPersonalia.skattetatenUtbetalinger =
            skattbarInntekt.oppgaveInntektsmottaker.map {
                FrontendSkattbarInntekt.skattUtbetaling(
                    it
                )
            }
        ...
        frontendPersonalia.administratorRoller =
            rolleService.hentKonfigurasjon(personalia.fnr)
    ...
    fun zipSoknad(@PathVariable fiksDigisosId: String): ResponseEntity<ByteArray> {
        ...
        val soknadJson = soknadService.hentDokument(
            fiksDigisosId,
            soknad.originalSoknadNAV!!.metadata
        )
        ...
        soknadsInfo.vedlegg
            .filterNot { vedlegg ->
                ...
                vedlegg.id in
                        (soknad.ettersendtInfoNAV?.ettersendelser?.map { it.vedleggMetadata }
                            ?: emptyList())
            }
            .forEach { vedlegg ->
    ...
    @GetMapping(
        "/mock-alt/ettersendelse/{fiksDigisosId}",
        produces = ["application/zip"]
    )
    ...
    private fun toVedlegg(dokument: DokumentInfo): FrontendVedlegg {
        val kanLastesned =
            soknadService.hentFil(dokument.dokumentlagerDokumentId) != null
